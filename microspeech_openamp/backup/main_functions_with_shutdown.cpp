...
{
	int status = k_sem_take(&data_sem, K_MSEC(1000)); // Add timeout

	if (status == 0 && !shutdown_requested) {
...
void app_rpmsg_tty(void *arg1, void *arg2, void *arg3)
{
	...
	if (ret) {
		...
		return; // Simple return instead of goto
	}
    ...
    while (tty_ept.addr != RPMSG_ADDR_ANY && !shutdown_requested) {
        // Add timeout to prevent infinite blocking
        if (k_sem_take(&data_tty_sem, K_MSEC(1000)) == 0) {
            if (tty_msg.len) {
                ...
                if (tty_msg.len == 3 && memcmp(tty_msg.data, "EOF", 3) == 0) {
                    ...
                    // Convert bytes to samples (file_size is in bytes, need sample count)
                    size_t audio_samples = file_size / sizeof(int16_t);
                    ret = micro_speech_process_audio("yes", file_buffer, audio_samples);
                    if (ret != 0) {
                    ...
                    // Signal shutdown after processing audio
                    snprintf(tx_buff, sizeof(tx_buff), 
                            "[Z] Processing complete. Shutting down...\n");
                    rpmsg_send(&tty_ept, tx_buff, strlen(tx_buff));
                    ...
                    if (file_size + tty_msg.len <= sizeof(file_buffer)) {
                        memcpy(((char*)file_buffer) + file_size, tty_msg.data, tty_msg.len);
                        ...
                        snprintf(tx_buff, sizeof(tx_buff),
                            "[Z] Buffer overflow! Shutting down.\n");
                        rpmsg_send(&tty_ept, tx_buff, strlen(tx_buff));
                        shutdown_requested = true;
                        break;
                    }
    ...
    // Send final message before destroying endpoint
    if (tty_ept.addr != RPMSG_ADDR_ANY) {
...
void setup() {
    ...
    k_thread_create(&thread_tty_data, thread_tty_stack, 5120,
        app_rpmsg_tty,
...
void loop() {
    ...
    if (shutdown_requested) {
        LOG_INF("Shutdown requested - exiting main loop");
        return; // Exit the loop to allow clean shutdown
    }
